...
def main():
    ...
    pg.draw.circle(enn, (255, 0, 0), (10, 10), 10) # 半径10の赤い円を作成
    pg.draw.circle(enn1, (0, 255, 0), (10, 10), 10) # 半径10の青い円を作成
    ...
    while True:
        ...
        sum_move = [0, 0] # 合計移動量の初期化 
        key_lst = pg.key.get_pressed()
        if key_lst[pg.K_UP]: sum_move[1] -= 5 # 上矢印
        if key_lst[pg.K_DOWN]: sum_move[1] += 5 # 下矢印
        if key_lst[pg.K_LEFT]: sum_move[0] -= 5 # 左矢印
        if key_lst[pg.K_RIGHT]: sum_move[0] += 5 # 右矢印
        
        screen.blit(bg_img, [0, 0])
        kk_img_rct.move_ip(sum_move[0], sum_move[1]) 
        if screen_judg(kk_img_rct) != (True, True):
            kk_img_rct.move_ip(-sum_move[0], -sum_move[1]) # 速度に応じて位置を移動させる
        # 動いていない場合
        if sum_move == [0, 0]:
